...
// Inline brand marks, keyed by the `type` stored in content.
const socialIcons = {
  linkedin: (
...
const Footer = () => {
  ...
  // A footer link is either an in-page section scroll or a router route.
  const renderLink = (link, i) =>
    link.kind === 'route' ? (
      <li key={i}><Link to={link.href}>{link.label}</Link></li>
    ) : (
      <li key={i}>
        <a href={`#${link.href}`} onClick={(e) => handleSectionNav(e, link.href)}>{link.label}</a>
      </li>
    );
      ...
      <div className="container">
        <div className="footer-grid">
            ...
            <div className="social-links">
              {footer.socials.map((s, i) => (
                <a
                  ...
                  aria-label={s.type}
                >
                  {socialIcons[s.type] || socialIcons.github}
                </a>
              ))}
            </div>
          </div>
          ...
          {footer.columns.map((col, i) => (
            <div className="footer-links-col" key={i}>
              <h4>{col.title}</h4>
